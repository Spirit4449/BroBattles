export const KEY_SLOTS = Object.freeze([
  ...
  ['dash','Dash',32],['attack','Attack',74],['special','Special',73],['interact','Mode interaction',69],
]);
...
export function normalizeBindings(raw) {
  ...
  raw={...raw, dash: raw.dash ?? 32};for(const [id] of KEY_SLOTS)if(id.endsWith('Alt')||id==='dash')raw[id]=DEFAULT_BINDINGS[id];
...
export function assignBinding(bindings,slot,code) {
  if(!(slot in DEFAULT_BINDINGS)||slot.endsWith('Alt')||slot==='dash'||!validKey(code))throw new Error('Use a letter, number, arrow, or Space.');
