...
export function renderSubmit(root,kind,onComplete) {
  ...
  form.onsubmit=async event=>{event.preventDefault();button.disabled=true;error.textContent='';try{const fields=Object.fromEntries(new FormData(form));const result=await api(kind==='feedback'?'/api/feedback':'/api/support/requests',{...fields,submissionKey});root.replaceChildren(element('h2','Thank you!'),element('p',`${kind==='feedback'?'Feedback':'Request'} #${result.id} received.`));if(kind==='support'){const a=element('a','View your requests');a.href='/help/requests';root.append(a);}onComplete?.(result);}catch(e){error.textContent=e.message;button.disabled=false;}};
}
export function renderInbox(root,{admin=false,kind='support'}={}) {
  ...
  async function detail(id) {
    ...
    try {
      ...
      const back=element('button','← Back to requests','pixel-menu-button');back.onclick=list;root.append(back,element('h2',data.request.subject),element('p',`#${id} · ${data.request.category} · ${data.request.status.replace('_',' ')}`));
  ...
  async function list() {
    ...
    root.style.minHeight=`${root.getBoundingClientRect().height}px`;
    try {
      const data=await api(`${base}?page=${page}&status=${encodeURIComponent(status)}&category=${encodeURIComponent(category)}`);if(requestGeneration!==generation)return;root.replaceChildren();
      const filters=element('div',null,'site-filter');
