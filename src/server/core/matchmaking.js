...
const {
  loadMatchData,
  deleteMatchBots,
} = require("../services/matchRosterService");
const {
...
const {
  ...
  pickCompositeGroup,
  pickGroup,
} = require("./matchmaking/teamBalancer");
...
function createMatchmaking({ io, db, gameHub = null, runtimeConfig = null }) {
  ...
  const readyCheckCoordinator = createReadyCheckCoordinator({
    ...
    cancelMatch,
    getMatchDataForGameRoom: (id) => loadMatchData(db, id),
    gameHub,
  });
  const assembly = createMatchAssemblyManager({
    db,
    io,
    partyStatus: PARTY_STATUS,
    lastProgress,
  ...
  const queueTicketManager = createQueueTicketManager({
    db,
    partyStatus: PARTY_STATUS,
  ...
  async function getRealUsernames() {
    const now = Date.now();
    if (now - lastRealUsernameFetch > 60000 || !cachedRealUsernames.length) {
      try {
        const rows = await db.runQuery(
          "SELECT name FROM users WHERE expires_at IS NULL AND name NOT LIKE 'Guest%' AND name != '' ORDER BY user_id DESC LIMIT 100",
        );
        if (rows && rows.length) {
          cachedRealUsernames = rows.map((r) => r.name).filter(Boolean);
          lastRealUsernameFetch = now;
        }
      } catch (_) {}
    }
    return cachedRealUsernames;
  }
  ...
  async function maybeStopLoop() {
    ...
    if (!Number(c) && loop && activity === observedActivity) {
      clearInterval(loop);
      loop = null;
      drafts.clear();
    }
  ...
  async function tick() {
    ...
    try {
      ...
      for (const items of groupBy(
        queued,
        (t) => `${t.mode_id}:${t.mode_variant_id}:${t.map}`,
      ).values()) {
        const selection = normalizeSelection({
          modeId: items[0].mode_id,
          modeVariantId: items[0].mode_variant_id,
          mapId: items[0].map,
        });
        ...
        if (config.enabled)
          for (const anchor of [...items]) {
            if (
              ...
              !stagedSeatCount(anchor, Date.now(), config)
            )
              continue;
            const cohort =
              ((Number(anchor.party_id || anchor.user_id) * 2654435761) >>> 0) %
              100;
            ...
            const signature = humans
              .map(
                (p) =>
                  `${p.user_id}:${p.team}:${p.char_class}:${p.level}:${p.trophies}`,
              )
              .join("|");
            ...
            if (!draft || draft.signature !== signature) {
              ...
              draft = {
                signature,
                seed,
                bots: createBotParticipants(humans, teamSize, {
                  seed,
                  realNames,
                }),
              };
            ...
            const count = stagedSeatCount(anchor, Date.now(), config);
            const staged = draft.bots.slice(0, count);
            if (staged.length === draft.bots.length) {
              await assembly.assembleAndReady(
                modeId,
                modeVariantId,
                mapId,
                picks,
                { teamSize, fillBots: true, ...draft },
              );
              drafts.delete(anchor.ticket_id);
              removePicks(items, picks);
            } else {
              await progress.emitProgressForBucket(
                modeId,
                modeVariantId,
                mapId,
                picks.map((p) => p.ticket),
                teamSize,
                { roster: [...humans, ...staged] },
              );
  ...
  async function queueLeave(args) {
    const result = await queueTicketManager.queueLeave(args);
    drafts.clear();
    return result;
  }
  ...
  async function createBotFilledMatch({
    ...
    const anchor = tickets.find((t) =>
      partyId
        ? Number(t.party_id) === Number(partyId)
        : !t.party_id && Number(t.user_id) === Number(userId),
    );
    ...
    const items = tickets.filter(
      (t) =>
        t.mode_id === anchor.mode_id &&
        t.mode_variant_id === anchor.mode_variant_id &&
        t.map === anchor.map,
    );
    const picks = pickGroup(items, teamSize, {
      partial: true,
    ...
    const result = await assembly.assembleAndReady(
      ...
      configuredPicks,
      {
        teamSize,
        fillBots: true,
        healthOverride: Number(botHealthOverride) === 9999999 ? 9999999 : null,
        realNames,
      },
    );
  ...
  async function cancelMatch(matchId, reason) {
    ...
    try {
      await deleteMatchBots(db, matchId);
    } catch (error) {
