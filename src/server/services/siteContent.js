...
function frame(title, description, body, urlPath) {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width, initial-scale=1"/>
    ...
    </head><body class="site-page ${urlPath==='/about'?'site-about':''}"><a class="site-skip" href="#main">Skip to content</a><header class="site-header"><div class="site-brand"><a href="/" class="lobby-wordmark"><img src="/assets/logos/wordmark.webp" alt="Bro Battles"/></a></div><a class="pixel-menu-button site-play" href="/">Play now</a></header>
    <main id="main" class="site-main">${body}</main><footer class="site-footer"><div class="site-footer-brand"><img src="/assets/logos/wordmark.webp" alt="Bro Battles"/><p>Small pixels. Big rivalries.</p><span>${escape(config.version)}</span></div><div><h3>Explore</h3><a href="/about">The game</a><a href="/about#modes">Game modes</a><a href="/news">News</a></div><div><h3>Player support</h3><a href="/help">Help Center</a><a class="pixel-menu-button site-secondary" href="/help/requests">My requests</a><a href="/feedback">Feedback</a></div><div><h3>The essentials</h3><a href="/privacy">Privacy Policy</a><a href="/terms">Terms of Service</a><a href="mailto:${config.supportEmail}">Contact us</a></div><div class="site-footer-bottom">© ${new Date().getFullYear()} Bro Battles <span>Made for your next “one more match.”</span></div></footer><script src="/bundles/site.bundle.js" defer></script></body></html>`;
}
function registerSitePages(app) {
  ...
  app.get('/about',(_req,res)=>res.send(frame('About','Fast browser battles. Pick your fighter and play with friends.',`
    <section class="site-hero" data-media-slot="about-hero"><div class="site-hero-copy"><span class="site-eyebrow">${escape(config.version)}</span><h1>One more match.<br/>One more rivalry.</h1><p>Outplay your rivals in Duels. Raid the vault in Bank Bust. Bring your crew and make the arena yours.</p><a class="pixel-menu-button site-play" href="/">Enter the arena</a></div><img src="/assets/lushy/lobbyBg.webp" alt="The Lushy Peaks arena"/></section>
  ...
  app.get('/help',(_req,res)=> {
    ...
    res.send(frame('Help Center','Answers and support for Bro Battles.',`<section class="site-help-hero"><span class="site-eyebrow">Bro Battles support</span><h1>How can we help?</h1><label class="site-search">Search help<input id="help-search" type="search" placeholder="Try accounts, controls, or purchases" autocomplete="off"/></label></section>
      <div class="site-help-actions"><a class="pixel-menu-button" href="/help/contact">Contact us</a><a class="pixel-menu-button site-secondary" href="/help/requests">My requests <span data-support-unread></span></a></div>
      <div class="site-grid" id="help-categories">${categories.map(category=>`<section class="site-panel site-help-card" data-help-category><img class="site-help-art" src="${helpArt[category]}" alt="" loading="lazy"/><h2>${escape(category)}</h2>${manifest.help.filter(item=>item.category===category).map(item=>`<a class="site-help-link" data-help-search="${escape(`${item.title} ${item.summary} ${category}`.toLowerCase())}" href="/help/${item.slug}">${escape(item.title)} <span aria-hidden="true">→</span></a>`).join('')}</section>`).join('')}</div><p id="help-empty" hidden>No matching answers. <a href="/help/contact">Contact us</a>.</p>`, '/help'));
  });
