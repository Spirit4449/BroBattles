...
Apply [migrations/2026-04-16_skins.sql](migrations/2026-04-16_skins.sql)
to support per-character selected skins and ownership unlock tracking.
...
## Production hardening (2026-09-10)

Apply [migrations/2026-09-10_production_hardening.sql](migrations/2026-09-10_production_hardening.sql) before starting this server version. The migration was applied and verified against the configured local database during this review. It adds session hashes, idempotent reward receipts, and a durable Stripe event inbox. Required existing tables must use InnoDB; the battle-log migration must also be applied.

Run `node scripts/apply-hardening-migration.cjs` to apply and verify both the battle-log and hardening migrations using `.env`. Reapplying is safe. Existing numeric identity cookies will require users to log in again. See [the deployment and recovery notes](docs/PRODUCTION_HARDENING.md).
...
## Public pages, support, and legal acceptance (2026-09-12)

Apply `migrations/2026-09-12_site_support.sql` before deploying the site release. No age or birthdate fields are collected. Settings are browser-local. The request owner is always derived from the authenticated session; support requires a permanent account. Feedback and closed support have 12-month retention.
