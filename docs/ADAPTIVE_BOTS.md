...
- Names come from an expanded pool featuring troll names (`SkillIssue`, `TouchGrass`, `DeleteTheGame`, etc.), gamer tags, procedural adjective-noun combinations (over 3,200 unique pool entries), a chance to sample registered real usernames from the database, and occasional guest default usernames (`Guest` or `GuestXXXXXX`), without duplicates among match participants. Characters are chosen uniformly from all six available characters, using default skins. Bot level is the rounded median human selected-character level; skill uses the mean human trophies.
...
The decision controller samples visible nearby opponents and projectiles, delays reactions, and scores attacks, spacing, recovery, regeneration, and pickups. Each bot has reproducible personality and aim variation. Difficulty interpolates through trophy anchors at 0, 500, 1,250 and 2,000, with reaction ranges of 300–420, 220–310, 150–220, and 105–170 ms respectively. Higher tiers also lead targets more accurately, dodge more reliably, reconsider tactics faster, and make fewer attack mistakes. Mistakes occur at attack opportunities, not as per-frame random inputs. High-tier strength remains a tuning target rather than a forced outcome.

Movement decisions persist for roughly 0.85–1.5 seconds, with a brief opening pause and short patrol/search pauses spaced about 7–12 seconds apart. Random pauses do not interrupt active fights. Bots hold useful firing positions instead of constantly pursuing the opponent's exact location. Character spacing, nearby enemies, travel cost, and height determine which ground they seek. Walking/drop routes compete with jumps; seeded route preferences and temporary avoidance of failed edges provide alternate paths.

Bots begin retreating at roughly 38–43% health, depending on tactical skill, and remain cautious until roughly 69–75%. Retreat scoring strongly favors ground that creates real distance from every nearby enemy and reevaluates immediately after another hit. Counterfire is limited to occasional close-pressure shots so regeneration can begin once the bot escapes. Useful reachable powerups can interrupt a fight, and a weakened opponent can draw focus away from a healthy target. Projectile dodges work at every trophy band with different success chances. Optional escape hops require simulated safe landings, and wall recovery includes brief slides and occasionally late jumps. A progress watchdog also handles missing routes and zero movement intent, while excluding deliberate pauses.

Charged supers are held briefly so the full meter and character-specific positioning create pressure. Ninja waits for aligned, weakened, or grouped targets; Thorg activates rage upon entering melee; Draven requires close or clustered enemies before anchoring Inferno; Wizard favors moments when the team needs buffs; Huntress prefers a stable mid/long-range volley; and Gloop hooks retreating or vulnerable targets from useful range. Bots keep the super while retreating or when these conditions are poor.

When a bot has at least a modest health lead over its target, it gives that wounded opponent higher target priority, stops optional kiting, and closes to a more aggressive version of its character's preferred range. Its own retreat rules still take priority.
...
Behavior counters include deliberate/positional idle time, stalled movement time, dodges, jumps, optional escape hops, wall pauses, target switches, retreats, and pickup goals. `tests/botBehavior.test.js` covers holding ground, retreat/counterfire, actual pickup collection, target switching, ranged perception/dodging, leading shots, and unreachable-target recovery. The seeded simulation includes powerup spawns and collection.
