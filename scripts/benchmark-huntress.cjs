// Compare six firing Huntresses under the same headless workload; not a GPU or hosting benchmark.
...
function run(version) {
  ...
  room.huntressCombatVersion = version; room.botControllers.clear();
  ...
  try {
    for(let tick=0;tick<3600;tick++) {
      ...
      for(const p of players) {
        ...
        if(tick%90===0&&p.ammoState.charges>0) {
          if(version===1){p.ammoState.charges--;p.ammoState.nextFireInMs=p.ammoState.cooldownMs;}
    ...
    times.sort((a,b)=>a-b);
    return {version,tickP95Ms:+times[Math.floor(times.length*.95)].toFixed(3),
...
for(let trial=0;trial<3;trial++) console.log(JSON.stringify({trial,baseline:run(1),huntressV2:run(2)}));
