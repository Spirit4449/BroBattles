...
(async()=>{
  ...
  try {
    ...
    await conn.query(fs.readFileSync(path.join(__dirname,'../migrations/2026-09-13_email.sql'),'utf8'));
    const [rows]=await conn.query("SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME IN ('account_emails','email_outbox')");
    if(rows.length!==2)throw new Error('Site migration verification failed');
    console.log('Email migration applied and both tables verified.');
